...
class SolarAssistantConfigFlow(ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user(
        ...
        return self.async_show_menu(
            step_id="user",
            menu_options=["local", "cloud", "modbus"],
        )
    ...
    # -- Path C: direct Modbus connection --
    ...
    async def async_step_modbus(
        ...
        errors: dict[str, str] = {}
        if user_input is not None:
            modbus_ip = user_input[CONF_MODBUS_IP].strip()
            modbus_port = user_input.get(CONF_MODBUS_PORT, 502)
            modbus_slave_id = user_input.get(CONF_MODBUS_SLAVE_ID, 1)
            ...
            try:
                ...
                if not client.connect():
                    ...
                    result = client.read_holding_registers(1, 1, slave=modbus_slave_id)
                    client.close()
                    if result.isError():
                        errors["base"] = "cannot_connect"
                    else:
                        await self.async_set_unique_id(f"modbus:{modbus_ip}:{modbus_port}")
                        self._abort_if_unique_id_configured()
                        return self.async_create_entry(
                            title=f"SolarAssistant Modbus ({modbus_ip})",
                            data={
                                CONF_AUTH_METHOD: AUTH_MODBUS,
                ...
                _LOGGER.exception("Modbus connection error")
                errors["base"] = "unknown"

        schema = vol.Schema({
            vol.Required(CONF_MODBUS_IP): str,
            vol.Optional(CONF_MODBUS_PORT, default=502): int,
            vol.Optional(CONF_MODBUS_SLAVE_ID, default=1): int,
        })
